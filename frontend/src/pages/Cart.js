...
const context = useAppContext(); // Use the custom hook
  ...
  return (
    <div className='container mx-auto'>
      <div className='text-center text-lg my-3'>
      {
       ...
       <p className='bg-slate-100 font-bold py-5'>No Data</p>
    )
     }
      </div>
...
<div className='flex flex-col lg:flex-row gap-10 justify-between p-4'>
  ...
  return(
    <div key={product?._id + "Add To Cart Loading"} className='w-full bg-slate-300 h-32 my-2 border-slate-300 rounded grid grid-cols-[128px,1fr]'>
...
<p className='text-red-600 font-medium text-lg'>{displayAEDCurrency(product?.productId?.sellingPrice)}</p>
<p className='text-slate-600 font-semibold text-lg'>{displayAEDCurrency(product?.productId?.sellingPrice * product?.quantity )}</p>
  </div>
<div className='flex items-center gap-3 mt-1'>
  <button className=' border border-red-600 text-red-600 hover:bg-red-600 w-6 h-6 hover:text-white flex justify-center items-center rounded' onClick={()=>decreaseQty(product?._id, product?.quantity)}>-</button>
  <span>{product?.quantity}</span>
  <button className=' border border-red-600 text-red-600 hover:bg-red-600 w-6 h-6 hover:text-white flex justify-center items-center rounded' onClick={()=>increaseQty(product?._id, product?.quantity)}>+</button>
...
<div className='flex items-center justify-between px-4 gap-2 font-modium text-lg text-slate-600'>
<p>quantity</p>
<p>{totalQty}</p>
  </div>
<div className='flex items-center justify-between px-4 gap-2 font-modium text-lg text-slate-600'>
